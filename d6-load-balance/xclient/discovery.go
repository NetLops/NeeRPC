...
const (
	...
	RoundRobinSelect                   // select using Robbin an algorithm
)

type Discovery interface {
	Refresh() error                      // refresh from remote registry	 // 从注册中心更新服务列表
	Update(servers []string) error       // 手动更新服务列表
	Get(mode SelectMode) (string, error) // 提供负载均衡策略，选择一个服务器实例
	GetAll() ([]string, error)           // 返回所有的服务实例
}

var _ Discovery = (*MultiServersDiscovery)(nil)

// MultiServersDiscovery is a discovery for multi servers without a register center
...
func NewMultiServersDiscovery(servers []string) *MultiServersDiscovery {
	...
	d.index = d.r.Intn(math.MaxInt32 - 1) // 避免从0开始
	return d
}
