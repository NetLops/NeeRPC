...
var DdefaultOption = &Option{
	...
	ConnectTimeout: time.Second * 10, // 10s represents Timeout
}
...
func (server *Server) findService(serviceMethod string) (svc *service, mtype *methodType, err error) {
	dot := strings.LastIndex(serviceMethod, ".") // example: Foo.Sum
	if dot < 0 {
...
func (server *Server) handleRequest(cc codec.Codec, req *request, sending *sync.Mutex, wg *sync.WaitGroup, timeout time.Duration) {
	defer wg.Done()
	called := make(chan struct{}) // 占位通知用的
...
const (
	connected        = "200 Connected to Nee RPC"
	defaultRPCPath   = "/_neerpc_"
	defaultDebugPath = "/debug/neerpc"
)
